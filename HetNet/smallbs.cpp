...
smallBS::smallBS(double x, double y, uint32_t id, Backhaul tt, Status st, typ ss)
...
// SBS::SBS(uint32_t id, double ptx)
// : the_thread()
// {
//     setID(id); 
//     m_TxP = ptx;
...
void smallBS::Start()
{
    ...
    the_thread = std::thread(&smallBS::ThreadMain,this);
...
// void SBS::setColor(std::size_t color)
// {
// 	m_color = color;
...
double smallBS::calculate_SNR_of_link(double x, double y)
{
    // SNR = S/N
 ...
 */
double smallBS::calculate_SINR_of_link(double x, double y, double interf)
{
    // SNR = S/N
    std::random_device rd;
    std::mt19937 gen(rd());
...
double smallBS::calculate_Interf_of_link(double x, double y)
{
    // SINR = S/(I+N)
    std::random_device rd;
    std::mt19937 gen(rd());
...
double smallBS::calculate_Rate_of_link(double x, double y)
{
...
double smallBS::calculate_distance_of_link(double x, double y)
{
    double dist = euclidean_dist(x, y, getX(), getY());
    return dist;
}

// void SBS::emit_update_parent()
// {
...
void smallBS::set_backhaul_Type(Backhaul st)
{
    m_bkhl = st;
...
// void SBS::add_to_load_BS(boost::shared_ptr<SBS> bs/*uint32_t id, point p*/)
// {
//     bool found = false;//= m_load_BS.find(id)!=m_load_BS.end();
//     
//     for(std::vector<boost::shared_ptr<SBS>>::iterator it=m_load_BS.begin(); it!=m_load_BS.end();++it)
//     {
//         boost::shared_ptr<SBS> mmB = (*it);
...
// void SBS::remove_from_load_BS(uint32_t id)
// {
...
// int SBS::get_load_BS_count()
// {
//     int s = m_load_BS.size(); 
//     return s;
// }

// void SBS::update_load_hops()
// {
